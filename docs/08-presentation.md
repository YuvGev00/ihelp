...
|---|---|---|
| 0–1 | **The problem + the product** — finding trustworthy help is slow and one-directional; iHelp reverses it: post a request, verified helpers compete on trust, speed, **and price** (each offer carries the helper's pricing stance — fixed price, price-after-the-job, or volunteer) | מה המוצר · איזו בעיה הוא פותר |
...
| 7–9 | **How it's built** — one-screen architecture (internal doc §1); "the database is the only authority" thesis; the 11-RPC + RLS + guard-trigger design; why Server Actions and no REST; the profiles split story | איך המערכת בנויה · מה הארכיטקטורה |
...
| 10–11 | **Tests** — the inverted pyramid: 29 integration tests that *attack* the DB as the wrong user; show test P10 (self-admin attempt) and X1 (assign-vs-withdraw race) failing closed; 10-second E2E clip | אילו בדיקות כתבתם |
...
3. **B:** open the same request, submit an offer — show the **three-stance
   pricing picker** (מחיר קבוע / מחיר ייקבע לאחר העבודה / בהתנדבות (חינם)),
   pick fixed **at ₪120** — and the sealed-bid note: B will never see
   competitors. 30 sec.
...
6. **A:** rate 5 stars → status **דורגה**; open Yossi's profile — the average
   moved. 30 sec.
7. **B (newest feature):** on a second seeded request where Yossi's offer was
   **מחיר ייקבע לאחר העבודה**, the completed request shows the
   **קביעת המחיר הסופי** panel — enter the amount, submit, and the price chip
   flips from מחיר ייקבע בסיום to the final ₪. 15 sec.
8. **Admin tab:** show the pending queue (Noa's application with prior-history
   panel), approve; one glance at moderation hide/unhide. 40 sec.
9. **Fallback:** if anything misbehaves live, the E2E recording
...
- [ ] Demo instance seeded, demo password set via `SEED_PASSWORD`, works in
      incognito (seed command documented in README)
