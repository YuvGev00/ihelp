...
**מסמך:** 1 of 11 (מפרט מוצר, שלב 2 של המטלה)
**סטטוס:** טיוטה לסקירה
...
- **מבקש** מפרסם בקשת עזרה: כותרת, תיאור, קטגוריה, תצלום אחד או יותר, ומיקום — **ללא בחירת תשלום/תמחור**. בקשה היא פשוט תיאור של מה שנדרש. **התמחור הוא כולו של העוזר**: כל הצעה מצהירה על אחת משלוש עמדות — **מחיר קבוע** עכשיו, **התנדבות** (חינם), או **"המחיר ייקבע לאחר העבודה"** (עבודות רבות, כמו של שרברב או חשמלאי, לא ניתן לתמחר לפני שהעוזר רואה את הבעיה; הסכום הסופי מוזן לאחר שהעבודה הושלמה). כל בקשה מקבלת את שלושתן.
...
| C3 | יצירת בקשת עזרה (למשתמשים מאומתי-זהות בלבד) עם כותרת, תיאור, קטגוריה, ≥1 תצלום (מועלה לאחסון), ומיקום — ללא בחירת תשלום (התמחור הוא של העוזר, לכל הצעה) | G1, G2, G4 |
...
1. המבקש ממלא כותרת, תיאור, קטגוריה, מעלה תצלום אחד לפחות, ומאשר את מיקום הבקשה (ברירת מחדל היא מיקום הפרופיל). דרישת התצלום היא מכוונת: היא מעלה את איכות הבקשה ואת מהימנותה, מרתיעה ספאם ופרסומים ברמת מאמץ נמוכה, ומאפשרת לעוזרים להעריך את היקף העבודה לפני ההצעה. עבור משימות שאין בהן דבר פיזי להראות (טופס, הסעה), כל תצלום רלוונטי מספק את הכלל — חיכוך מקובל בתמורה לזרם (feed) איכותי יותר.
...
- בקשות עזרה: יצירה/עריכה/ביטול על ידי הבעלים, קטגוריה, ≥1 תצלום, מיקום, מחזור חיים מלא של §9 (ללא בחירת תשלום — התמחור הוא של העוזר)
